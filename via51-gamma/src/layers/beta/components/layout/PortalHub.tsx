 ...
 * DESC: Fachada tripartita del ecosistema VÍA51.
 */
...
export const PortalHub: React.FC = () => {
  // 1. CORRECCIÓN CRÍTICA: Destructurar el hook para obtener el dato real
  const { tenant, loading } = useTenant();

  // 2. SALVAGUARDA DE SOBERANÍA: No renderizar si no hay datos
  if (loading) {
    ...
    <div className="min-h-screen bg-black text-white font-mono selection:bg-white selection:text-black">
      ...
      <nav className="border-b border-white/10 p-4 flex justify-between items-center bg-zinc-900/50 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center gap-4">
          <span className="text-xl font-black tracking-tighter uppercase">VÍA51 // {tenant?.slug || 'CORE'}</span>
        ...
        <div className="text-[10px] text-right uppercase opacity-40">
          {/* Línea 24 blindada con Optional Chaining y valor por defecto */}
