...
export default function App() {
    ...
    useEffect(() => {
        const checkHub = async () => {
            try {
                // Forzamos un timeout de 5s para no quedar colgados
    ...
    const triggerSinapsis = async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/v1/gatekeeper`, {
                ...
                body: JSON.stringify({
                    v51_dna: { node: "NODE-ALFA-0", seq: "A-28", env: "LAB", pulse: Date.now() },
    ...
    return (
        <main className="h-screen bg-black text-white flex items-center justify-center font-sans p-6 overflow-hidden">
            <div className="w-full max-w-[420px] h-[750px] bg-zinc-950 border-[12px] border-zinc-900 rounded-[3.5rem] relative flex flex-col overflow-hidden shadow-2xl">
                
                {/* CABECERA CON RESPIRO (pt-16) */}
                <div className="p-8 pt-16 pb-4 flex justify-between items-center">
                ...
                <div className="p-8 text-center">
                    <p className="text-[8px] text-zinc-700 uppercase tracking-[0.4em]">Antigravity System // A-28</p>
                </div>
            </div>
        </main>
    );
